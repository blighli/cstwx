		...
		animate: function(){
			...
			setTimeout(function(){
				...
				if(!isRevert){
				    ...
				    var next1 = cur_li.next();
				    if(next1.length){

				    	// 判断是否已处理，未处理则处理
				    	var temp_dom = next1.find('.li_bg').eq(0);
				    	var temp_bginfo = 'url(' + temp_dom.data('bginfo') + ')';
				    	if(temp_dom.css("background-image") != temp_bginfo){
				    		preload(temp_dom.data('bginfo'));
				    	    temp_dom.css( { "background-image" : temp_bginfo } );
		                    temp_dom.css( { "background-size"  : '100% 100%' } );
				    	}
				    	...
				    	if(next2.length){
				    	    ...
				    	    // 判断是否已处理，未处理则处理
				    	    temp_dom = next2.find('.li_bg').eq(0);
				    	    temp_bginfo = 'url(' + temp_dom.data('bginfo') + ')';
				    	    if(temp_dom.css("background-image") != temp_bginfo){
				    	    	preload(temp_dom.data('bginfo'));
				    	        temp_dom.css( { "background-image" : temp_bginfo } );
		                        temp_dom.css( { "background-size"  : '100% 100%' } );
				    		...
				    		if(next3.length){
				    	        ...
				    	        // 判断是否已处理，未处理则处理
				    	        temp_dom = next3.find('.li_bg').eq(0);
				    	        temp_bginfo = 'url(' + temp_dom.data('bginfo') + ')';
				    	        if(temp_dom.css("background-image") != temp_bginfo){
				    	        	preload(temp_dom.data('bginfo'));
				    	            temp_dom.css( { "background-image" : temp_bginfo } );
